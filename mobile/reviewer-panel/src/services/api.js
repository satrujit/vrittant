...
// Route by hostname so UAT site → UAT backend, prod site → prod backend.
// Same dist is deployed to both Firebase sites.
const API_BASE = (typeof window !== 'undefined' && window.location.hostname === 'vrittant-uat.web.app')
  ? 'https://vrittant-api-uat-pgvufpchiq-el.a.run.app'
...
export async function verifyOtp(phone, otp, reqId = '') {
  return apiFetch('/auth/verify-otp', { method: 'POST', body: JSON.stringify({ phone, otp, req_id: reqId }) });
}

export async function resendOtp(phone, reqId = '') {
  return apiFetch('/auth/resend-otp', { method: 'POST', body: JSON.stringify({ phone, req_id: reqId }) });
}
...
export async function fetchAdvertisers({ q, limit = 25 } = {}) {
  return apiFetch(`/admin/advertisers${buildQuery({ q, limit })}`);
}

export async function createAdvertiser(data) {
  return apiFetch('/admin/advertisers', {
    method: 'POST',
    body: JSON.stringify(data),
  });
}
...
export async function fetchAds(params = {}) {
  return apiFetch(`/admin/advertisements${buildQuery(params)}`);
}

export async function fetchAd(id) {
  return apiFetch(`/admin/advertisements/${id}`);
}

export async function createAd(data) {
  return apiFetch('/admin/advertisements', {
    method: 'POST',
    body: JSON.stringify(data),
  });
}
...
export async function deleteAd(id) {
  return apiFetch(`/admin/advertisements/${id}`, { method: 'DELETE' });
}

export async function changeAdStatus(id, status, note = '') {
  return apiFetch(`/admin/advertisements/${id}/status`, {
    method: 'PUT',
    body: JSON.stringify({ status, note }),
  });
}

export async function uploadAdAttachment(adId, file, label = '') {
  const token = getAuthToken();
  const formData = new FormData();
  formData.append('file', file);
  if (label) formData.append('label', label);
  const url = label
    ? `${API_BASE}/admin/advertisements/${adId}/attachments?label=${encodeURIComponent(label)}`
    : `${API_BASE}/admin/advertisements/${adId}/attachments`;
  const resp = await fetch(url, {
    method: 'POST',
    headers: { ...(token ? { Authorization: `Bearer ${token}` } : {}) },
    body: formData,
  });
  if (!resp.ok) throw new Error(`Upload failed: ${resp.status} ${await resp.text().catch(() => '')}`);
  return resp.json();
}

export async function deleteAdAttachment(adId, attId) {
  return apiFetch(`/admin/advertisements/${adId}/attachments/${attId}`, { method: 'DELETE' });
}

export async function fetchReviewQueue({ for_date, include = 'both', offset = 0, limit = 100 } = {}) {
  return apiFetch(`/admin/review-queue${buildQuery({ for_date, include, offset, limit })}`);
}
