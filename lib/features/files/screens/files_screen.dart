...
// ═══════════════════════════════════════════════════════════════════════════
// Tab Bar — coral underline on active
// ═══════════════════════════════════════════════════════════════════════════

class _FilesTabBar extends StatelessWidget {
  final TabController controller;
  final List<String> tabs;

  const _FilesTabBar({required this.controller, required this.tabs});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: TabBar(
        ...
        labelColor: AppColors.vrCoral,
        unselectedLabelColor: t.mutedColor,
        ...
        indicatorColor: AppColors.vrCoral,
        indicatorWeight: 2.5,
...
// ═══════════════════════════════════════════════════════════════════════════
// All Files View — flat list of every file
// ═══════════════════════════════════════════════════════════════════════════

class _AllFilesView extends StatelessWidget {
  final List<Map<String, dynamic>> files;
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.separated(
      ...
      itemBuilder: (_, i) => _FileListTile(file: files[i]),
    );
  }
}
...
class _FileListTile extends ConsumerWidget {
  ...
  IconData get _icon {
    final type = file['media_type'] as String? ?? '';
    switch (type) {
      case 'audio':
  ...
  void _openFile(BuildContext context, WidgetRef ref) {
    ...
    if (type == 'photo') {
      ...
      // Audio, video, document — download then open natively
      _downloadAndOpen(context, ref, _fullUrl(url), filename);
    }
  }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final size = _formatSize(file['size'] as int? ?? 0);
    final storyHeadline = file['story_headline'] as String? ?? '';
    final mediaType = (file['media_type'] as String? ?? '').toUpperCase();
    final createdAt = file['created_at'] as String?;
    final dateStr = createdAt != null ? _shortDate(createdAt) : '';
      ...
      child: Padding(
        ...
        child: Row(
          children: [
                ...
                children: [
                  ...
                  Text(
                    [
                      ...
                      if (storyHeadline.isNotEmpty) storyHeadline,
                    ].join(' · '),
                    style: GoogleFonts.plusJakartaSans(
                      fontSize: 11,
                      fontWeight: FontWeight.w400,
            ...
            Icon(LucideIcons.chevronRight, size: 18, color: t.mutedColor),
          ],
        ),
      ),
    );
...
// ═══════════════════════════════════════════════════════════════════════════
// Recent Files View — sorted by date
// ═══════════════════════════════════════════════════════════════════════════
...
class _RecentFilesView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.separated(
      ...
      itemBuilder: (_, i) => _FileListTile(file: sorted[i]),
    );
  }
}

// ═══════════════════════════════════════════════════════════════════════════
// Starred Files View — placeholder for now
// ═══════════════════════════════════════════════════════════════════════════

class _StarredFilesView extends StatelessWidget {
  final String title;
  final String subtitle;

  const _StarredFilesView({
    required this.title,
